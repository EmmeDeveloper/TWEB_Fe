...
import { useStore, updateFilter, getCoursesRepetitions } from '../../StateService.js'
  ...
  <div class="d-flex w-100 row mx-0 my-3">
    <template v-if="filterCollapsed">
      <div class="d-flex col-11 gap-2 py-1 align-center flex-wrap">
        <span class="fs-5" v-if="Object.keys(teachings).length == 0"
          >Nessuna materia selezionata</span
        >
        <div
          class="d-flex px-2 py-1 border rounded-pill align-center bg-primary-12"
          v-for="(prof, course) in teachings"
          :key="prof"
        >
          ...
          <i
            class="align-center bg-primary fs-5 justify-center mdi mdi-close rounded-circle text-center text-white pointer"
            style="width: 2rem; height: 2rem; line-height: 1.7"
            @click="removeRapid(course)"
          ></i>
        </div>
      </div>
      ...
      <div class="d-flex flex-column">
        ...
        <div class="d-flex flex-wrap gap-2">
          <template v-for="(profs, subject) in state.teachings" :key="subject">
            <FilterChip
              :selected="tempSelected[subject]?.length > 0"
              :enabled="true"
              @click="toggleTeaching(subject)"
            ...
            <FilterChip
              ...
              :key="professor.id"
              :enabled="true"
              :selected="tempSelected[subject]?.includes(professor.id)"
