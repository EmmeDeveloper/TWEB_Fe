  ...
  <nav class="align-center border-bottom d-flex justify-content-between px-3 w-100">
    <div class="d-flex gap-4 ps-3">
      <template v-for="p in pages" :key="p.page">
        <div
          v-if="p.condition"
          class="align-center d-flex gap-2 pointer p-2"
          :class="{ 'current-page': p.page == props.page }"
          @click="changePage(p.page)"
        >
          <i class="mdi fs-4" :class="'mdi-' + p.icon"></i>
          <span class="fs-7 d-none d-md-block">{{ p.name }}</span>
        </div>
      </template>
    </div>
    <div class="d-flex align-center">
      <div
        v-if="user == null"
        class="align-center border-primary border border-2 d-flex px-1 py-1 rounded-pill pointer"
        @click="loginClicked"
      >
        <div
